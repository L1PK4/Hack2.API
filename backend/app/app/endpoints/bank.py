...
def change_icon(
        new_icon: UploadFile | None = File(None),
        db: Session = Depends(deps.get_db),
        current_user: models.User = Depends(
            in_auth_session(deps.get_current_active_superuser)),
        bank_id: int = Path(...),
    ...
    bank = crud.crud_bank.bank.get(db=db, id=bank_id)
    if bank is None:
        raise UnfoundEntity(message="Банк не найден", num=1)

    res = crud.bank.change_content(
        db=db,
        obj=bank,
        content=new_icon
    )
    if res != 0:
        raise UnprocessableEntity("Не удалось изменить иконку")
